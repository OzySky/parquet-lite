...
function decodePages(buffer, opts) {
  ...
  while (cursor.offset < cursor.size && (!opts.num_values || data.dlevels.length < opts.num_values)) {
    ...
    switch (pageHeader.type) {
      case parquet_thrift.PageType.DATA_PAGE:
        valueEncoding = pageHeader.data_page_header.encoding;
        break;
      case parquet_thrift.PageType.DATA_PAGE_V2:
        valueEncoding = pageHeader.data_page_header_v2.encoding;
        break;
    }

    if (opts.dictionary && valueEncoding == parquet_thrift.Encoding.PLAIN_DICTIONARY) {
