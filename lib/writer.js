...
class ParquetWriter {
  ...
  static async openFile(schema, path, opts) {
    ...
    return ParquetWriter.openStream(schema, file, opts, file);
  }
   ...
   */
  static async openStream(schema, outputStream, opts, file) {
    if (!opts) {
      opts = {};
    }
  ...
  constructor(schema, envelopeWriter, opts) {
    ...
    try {
      ...
      const o = this.offset;
      this.envelopeWriter.writeFooter(this.userMetadata)
      this.offset = o;
    } catch (err) {
  ...
  async appendRow(row) {
    ...
    if (this.rowBuffer.rowCount >= this.rowGroupSize) {
      // console.log('107:', this.rowBuffer.pageRowCount, this.envelopeWriter.pageSize, this.rowBuffer.rowCount)
  ...
  async close() {
    ...
    this.closed = true;
    process.removeListener('beforeExit', this.onExit)
  ...
  set onflush(func) {
    typeof func === 'function' && this._onflush.push(func)
  }

  async flush(finalizeRows = false, finish) {
    finish ??= finalizeRows
    const writer = this.envelopeWriter
    if (!finalizeRows) {
      writer.originalOffset = writer.offset || 0
      writer.originalRowCount = writer.rowCount || 0
      writer.originalRowGroups = parquet_util.clone(writer.rowGroups)
    }

    if (this.rowBuffer.rowCount > 0 || !finish) {
      if (this.rowBuffer.rowCount > 0) {
        const rows = finalizeRows ? this.rowBuffer : parquet_util.clone(this.rowBuffer)
        encodePages(this.schema, rows, {useDataPageV2: writer.useDataPageV2});
        await writer.writeRowGroup(rows);
        if (finalizeRows) {this.rowBuffer = {}}
      }

      if (finalizeRows && !finish) {
        writer.originalOffset = writer.offset || 0
      }
      await writer.writeIndex(writer.rowGroups);
      await writer.writeFooter(this.userMetadata);
    }

    this._onflush.forEach(f => f(this))

    if (!finalizeRows) {
      writer.offset = writer.originalOffset
      writer.originalOffset = undefined
      writer.rowCount = writer.originalRowCount
      writer.rowGroups = writer.originalRowGroups
      writer.originalRowCount = writer.originalRowGroups = undefined
    } else if (!finish) {
      writer.offset = writer.originalOffset
      writer.originalOffset = undefined
    }
...
class ParquetEnvelopeWriter {
   ...
   */
  static async openStream(schema, outputStream, opts, file) {
    let writeFn = outputStream.write.bind(outputStream);
    let closeFn = outputStream.close.bind(outputStream);
    return new ParquetEnvelopeWriter(schema, writeFn, closeFn, 0, opts, file);
  }

  constructor(schema, writeFn, closeFn, fileOffset, opts, file) {
    this.file = file
    this.schema = schema;
    this.write = writeFn,
    this.close = closeFn;
  ...
  writeSection(buf) {
    ...
    return this.write(buf, undefined, undefined, start)
  }
   ...
   */
  writeFooter(userMetadata, flush) {
    if (!userMetadata) {
      userMetadata = {};
    }
    ...
    return this.writeSection(encodeFooter(this.schema, this.rowCount, this.rowGroups, userMetadata), flush);
  };
